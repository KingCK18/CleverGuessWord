...
def createDisplayString(lettersGuessed, missesLeft, guessedWordAsList):
    ...
    lettersGuessed = set(sorted(lettersGuessed))
...
def runGame(filename):
    f = open(filename, 'r')
    line = f.readlines()
    data = [word.strip() for word in line]
    wins = 0
    losses = 0
    ...
    while "_" in guessed_word_as_list and misses_allowed > 0:
        display_string = createDisplayString(letters_guessed, misses_left, guessed_word_as_list)
        guessed_letter = handleUserInputLetterGuess(letters_guessed, display_string)
        ...
        if not result[0] == False:
            misses_allowed -= 1


        if debug:
            print(f"(word is: {secret_word})")
            print(f"# possible words: {str(len(data))}")
    ...
    if "_" not in guessed_word_as_list:
        ...
        losses += 1
        print("You made:", len(letters_guessed), "guesses with:", misses_allowed, "misses")
